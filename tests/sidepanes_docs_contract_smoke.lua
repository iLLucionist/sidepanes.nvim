...
for _, item in ipairs({
    ...
    "after_open",
    "before_send",
    'The `"before_send"` timing is respected',
    "floating scratch prompt",
    "visual `aa` / `ax` opens the editor first",
    "manual `M` / `<Tab>` target choice",
    "does not prompt twice",
    "Refactor And Maintainability",
    "pure policy and route helpers",
    "target resolver",
    "session/status snapshot helpers",
    "controller and executor adapters",
    "lua/sidepanes/panes/ask/*",
    "behavior matrices",
    "mapping-zone coverage",
    "fed-key paths",
    "SidepanesAskAppend",
    "SidepanesAskStatus",
    "SidepanesSubmitQuestion",
    "SidepanesVersion",
    "`:checkhealth sidepanes` now reports",
    "append_to_ask(opts)",
    "submit_ask_pane()",
    "show_ask_pane(opts)",
    ":Sidepanes ask",
    "Setup validation now understands the `ask` and `help` config groups",
    "markdown.reload_badge.min_display_ms",
    "returning focus to an already-open Markdown side pane",
    "[RELOADED]",
    "Selection:",
    "exact duplicate file/range citations",
    "Plain normal-mode `q` is not mapped",
    "`:q` cancels empty drafts",
    "preserves modified drafts",
    "sends after `:w`",
    "`:q!` always cancels",
...
for _, item in ipairs({
    ...
    "## Tutorial",
    "## Mappings",
    "current grouped v0.4 configuration shape",
