...
under Neovim's state directory. Registry writes use an atomic rename plus a
lock directory and merge with any existing registry entries so independent
Neovim instances do not casually clobber each other. If Neovim or the plugin
crashes while holding the write lock, Sidepanes treats the lock as stale after
`terminal.resume.store_lock_stale_ms` and recovers it. Reads stay lock-free
because a registry reader sees either the previous complete file or the next
complete file. Sidepanes does not resume arbitrary latest project transcripts
on first open.
...
Custom resolvers receive `resolver(tool_name, ctx, opts)`. During capture,
`opts.purpose` is `"capture"` and the resolver can return a session id string or
a table such as `{ session_id = "...", evidence = { resolver_state = ... } }`.
For resolver-sourced remembered records, Sidepanes calls the resolver again
with `opts.purpose = "validate"` and `opts.remembered` before reuse. Return the
same session id, `true`, or `{ valid = true }` to keep the record; return
`false`, `nil`, or a different id to make Sidepanes clear it and start fresh.
...
process exits quickly with a non-zero status, Sidepanes clears the stale resume
id and starts the tool fresh once by default.
...
to provide a custom capture source. Set
`terminal.resume.store_lock_timeout_ms` if you want registry saves to wait
longer or fail faster under multi-Neovim contention. Set
`terminal.resume.store_lock_stale_ms` if your environment regularly suspends
Neovim for long periods or if you want crash recovery to happen sooner. Set
`terminal.resume.failure_timeout_ms` for CLIs that reject stale resume ids more
slowly, and set `terminal.resume.failure_action` to `"notify"` or `"ignore"` if
you do not want Sidepanes to automatically start fresh after a quick failed
resume.
