...
| Run focused ask tests, `tests/run_checks.sh fast`, `tests/run_checks.sh full`, `illu.nvim` smoke when mappings/local integration are affected, and `git diff --check`. | Focused ask/personal/submit regression, fast checks, full checks, and `git diff --check` were run before audit pass 1 found the `:w` gap; focused ask checks were rerun after the gap fix, and fast/full checks will be rerun before closeout. `illu.nvim` smoke is not applicable unless the `:w` command-path fix is judged to affect local config integration. | Focused ask run passed with 47 selected tests after `ec1d381`; previous `tests/run_checks.sh fast` and `tests/run_checks.sh full` passed with 166 regression tests before the gap fix; post-gap fast/full rerun pending. | Verification results are recorded below; CHANGELOG notes the `:w` command-path fix. | Run the same checks locally if desired. | `ec1d381`; verification rerun pending | In Progress |
...
- `tests/run_checks.sh fast` passed with 166 regression tests before the `:w`
  audit-gap fix; post-gap rerun pending.
- `tests/run_checks.sh full` passed with 166 regression tests and real CLI smoke
  before the `:w` audit-gap fix; post-gap rerun pending.
- `git diff --check` passed.
