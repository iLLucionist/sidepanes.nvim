...
| Extract pure selectors for: | `ask_session.snapshot()`, `status_data()`, `lifecycle_facts()`, and `format_title()` provide the pure selector surface and runtime callers now use that surface. | Direct selector tests cover every selector row below; focused regression passed with 164 tests. | This roadmap; no public docs change applies unless selector output becomes public status text. | Manual snapshot/status review. | `0c1054c`, `7aad91e`, `170eecb` | Done |
