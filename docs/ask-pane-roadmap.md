...
| `ask-q-written` | `:q` | `:quit` | ask pane | `draft_written -> sending_terminal -> sent` | Send written prompt, switch to target terminal, and clear ask state. | `tests/sidepanes_regression.lua` "ask pane command-line q sends after write" |
...
| `ask-write-draft` | `:w` | none | ask pane | `draft_modified`, `draft_written`, `send_failed -> draft_written` | Cache current prompt, mark buffer unmodified, stay in ask pane, and update winbar. | `tests/sidepanes_regression.lua` "question write without quit does not send" and ask-pane send mapping tests |
...
| `ask-write-quit-draft` | `:wq` | `:wq!`, `:x`, `:xit`, `:exit` | ask pane | `draft_modified`, `draft_written`, `send_failed -> draft_written`, `sending_picker`, `sending_terminal`, `sent`, `send_failed` | Write current prompt and send; on success switch to target terminal and clear ask state; on target failure warn and keep draft. | `tests/sidepanes_regression.lua` "ask pane command-line mapping cancels q and sends wq through internal callbacks" and "pane-mode ask sends accumulated prompt and clears state" |
...
| `markdown-pane-heading-and-ask` | Markdown pane | normal | `headings`, `ask_pane` | `fm`, `ap` | Open heading picker or switch to ask pane. | `tests/sidepanes_regression.lua` "pane-local mappings are configurable" and zone matrix regression |
...
| `terminal-pane-ask-and-toggles` | terminal pane | normal | `ask_pane`, `toggle_terminal`, `toggle_terminal_alt` | `ap`, `<leader>gg`, `<C-g>` | Open ask pane or return to Markdown/terminal counterpart. | `tests/sidepanes_regression.lua` "pane-local slot maps exist on markdown and terminal panes" and zone matrix regression |
...
| Include global mappings, pane-local mappings, visual mappings, and commands: | Mapping matrix rows cover global, pane-local, visual, and command modes; fixture `required_mappings`, `required_commands`, and `planned_commands`. | Docs contract checks fixture vocabulary and row IDs; zone matrix regression checks active installed maps and registered/planned command boundaries. | This roadmap matrix and `CHANGELOG.md` Unreleased Added entry. | Manual checks listed under this slice cover global, pane-local, visual, and command paths. | Done |
...
| `SidepanesAsk`, `SidepanesAskAppend`, `SidepanesSubmitQuestion`, `SidepanesAskStatus`, and `SidepanesVersion` once implemented. | Matrix rows `ask-zone-commands` and `ask-zone-planned-commands`; fixture required commands and planned commands. | Zone matrix regression verifies current commands are registered and planned commands are not registered early; docs contract verifies planned command rows exist. | This roadmap matrix. | Manual command checks for implemented commands; planned commands wait for slices 20 and 21. | Done |
...
- Add one central ask action policy module that owns action predicates and
  planning:
  - classify command-line text such as `:q`, `:q!`, `:wq`, `:x`, `:xit`, and
...
| classify command-line text such as `:q`, `:q!`, `:wq`, `:x`, `:xit`, and `:exit` into lifecycle intents. | `lua/sidepanes/ask_policy.lua` `commandline_intent()`; `lua/sidepanes/ask_pane.lua` command-line handler calls it. | Direct policy regression covers every listed command and alias; ask-pane command-line regression still covers callback command strings. Full checks passed. | This slice definition and behavior matrix. | Run the listed commands in the ask pane. | Done |
...
| Roadmap bullet | Implementation reference | Automated test reference, or explicit reason no automated test applies | Documentation reference, or explicit reason no docs change applies | Manual acceptance test reference | Commit reference | Status |
| --- | --- | --- | --- | --- | --- | --- |
| Split or group the large regression coverage so ask behavior can be audited by layer: | Planned: reorganize ask-focused regression coverage into explicit layer sections or smaller files, depending on harness cost. | Planned: focused ask regression run should make layer ownership visible. | This roadmap; no public docs change expected because test organization is internal. | Run the focused ask test group and confirm failures point to the relevant layer. | Pending | In Progress |
| policy predicate/action-plan tests. | Planned: keep or group `sidepanes.ask_policy` / controller action-plan coverage under a policy layer. | Planned: existing policy/action-plan assertions plus any renamed grouped coverage. | This roadmap; no public docs change expected because this is test architecture. | Confirm policy failures localize to decision behavior rather than key feeding. | Pending | In Progress |
| snapshot/selector tests. | Planned: keep or group `sidepanes.ask_session` selector coverage under a snapshot layer. | Planned: existing direct snapshot/selector assertions should remain focused and discoverable. | This roadmap; no public docs change expected because this is test architecture. | Confirm snapshot failures localize to selector facts/labels. | Pending | In Progress |
| executor tests with fake dependencies. | Planned: group controller/executor tests that use fake dependencies under an executor layer. | Planned: fake-dependency executor tests should remain direct and not depend on fed keys. | This roadmap; no public docs change expected because this is test architecture. | Confirm executor failures localize to effect execution. | Pending | In Progress |
| command-line adapter tests. | Planned: group command-line adapter and command-line fed-key coverage separately. | Planned: existing adapter callback tests stay only where they prove translation/registration, paired with fed-key behavior where user-visible. | This roadmap; no public docs change expected because behavior should not change. | Run command-line `:q`, `:q!`, `:w`, and `:wq` ask workflows. | Pending | In Progress |
| keymap registration tests. | Planned: keep callback assertions only for registration/config wiring and label them accordingly. | Planned: registration tests should assert maps/commands exist without serving as sole behavior coverage. | This roadmap; no public docs change expected because mappings do not change. | Inspect mapped keys in the relevant pane zones. | Pending | In Progress |
| fed-key user-path tests. | Planned: add or regroup fed-key tests for behavior-sensitive ask mappings and command paths. | Planned: fed-key coverage should exercise synthesized user keypaths for lifecycle behavior. | This roadmap; no public docs change expected unless behavior changes. | Press the real mappings listed in the behavior-sensitive coverage table. | Pending | In Progress |
| end-to-end smoke tests. | Planned: keep broader smoke coverage separate from unit/layer tests. | Planned: focused and full checks should continue to run smoke coverage. | This roadmap; no public docs change expected because smoke coverage is internal. | Run the focused ask test group plus fast/full checks. | Pending | In Progress |
| Reduce the 7k-plus-line catch-all regression file by moving ask-focused tests into smaller files when the test runner supports it, or by adding clear grouped helpers/sections when a split would create more harness churn than it removes. | Planned: inspect the test runner before choosing file split versus in-file grouped helpers/sections. | Planned: test count and focused ask run should stay stable after reorganization. | This roadmap; no public docs change expected because this is internal test organization. | Run focused ask tests and confirm output remains understandable. | Pending | In Progress |
| Identify callback assertions that test behavior-sensitive paths, especially ask-pane command-line and mapping paths, and pair them with fed-key coverage or replace them with fed-key coverage where Neovim can synthesize the path. | Planned: audit callback tests around ask command-line, ask send mappings, submit mappings, and pane-zone mappings. | Planned: every behavior-sensitive callback-only path should either gain fed-key coverage or an explicit no-fed-key reason. | This roadmap and coverage table created in this slice. | Compare each callback-sensitive path to the matrix and perform the real interaction manually. | Pending | In Progress |
| Add a behavior-sensitive mapping coverage table in tests or docs that lists every ask/non-ask mapping path and whether it has: | Planned: add a machine-checkable test-side table or roadmap/docs table linked to matrix rows. | Planned: docs-contract or regression coverage should verify the table does not drift where practical. | This roadmap; table may live in tests if that is the best source of truth. | Use the table as the manual acceptance checklist source. | Pending | In Progress |
| registration coverage. | Planned: coverage table will identify registration assertions for each mapping/command path. | Planned: test-side table assertions should cover registration column completeness. | Coverage table produced by this slice. | Confirm each mapping exists in the expected zone. | Pending | In Progress |
| direct policy/state coverage. | Planned: coverage table will identify direct policy/state assertions for each behavior path. | Planned: table assertions should ensure behavior-sensitive rows name a policy/state test or explicit exception. | Coverage table produced by this slice. | Confirm behavior expectations match the behavior matrix. | Pending | In Progress |
| fed-key coverage. | Planned: coverage table will identify fed-key tests for each behavior-sensitive path where applicable. | Planned: table assertions should ensure fed-key coverage is present or explicitly not applicable. | Coverage table produced by this slice. | Press each keypath in Neovim where fed-key applies. | Pending | In Progress |
| an explicit reason fed-key coverage does not apply. | Planned: coverage table will include a no-fed-key reason for unsupported or registration-only paths. | Planned: table assertions should reject missing coverage and missing no-fed-key reasons. | Coverage table produced by this slice. | Review exceptions before slice closeout. | Pending | In Progress |
| Convert callback-only tests for user-visible behavior into fed-key tests where Neovim can synthesize the key path reliably. | Planned: replace or pair callback-only ask behavior tests with fed-key tests. | Planned: fed-key tests should cover the converted user-visible behavior. | This roadmap; no public docs change expected because behavior should not change. | Perform equivalent real keypress workflows. | Pending | In Progress |
| Keep callback tests only for registration, normalized configuration, and narrow paths that cannot be fed realistically; record the reason near the test. | Planned: annotate or restructure remaining callback tests so their non-user-path role is clear. | Planned: regression coverage should include explicit reasons or table entries for callback-only cases. | This roadmap; no public docs change expected because this is test design. | Review remaining callback tests for scope. | Pending | In Progress |
| Ensure fed-key tests cover normal, insert, terminal, and command-line zones affected by ask lifecycle behavior. | Planned: audit and add fed-key coverage for affected mode/zone combinations. | Planned: fed-key tests should cover normal ask mappings, insert ask mappings, terminal/Markdown guarded quit mappings, and command-line ask lifecycle paths. | This roadmap; no public docs change expected unless behavior changes. | Repeat `qq` / `<leader>qq` and submit checks across the listed zones. | Pending | In Progress |
| Ensure tests compare expected behavior against the behavior matrix and mapping zone matrix rather than repeating stale implementation assumptions. | Planned: test helpers or coverage table should reference `tests/ask_pane_behavior_matrix.lua` and `tests/ask_pane_mapping_zone_matrix.lua`. | Planned: regression/docs-contract coverage should assert table expectations from matrix data where practical. | This roadmap; source matrices remain the documentation reference. | Compare manual workflows to the matrices, not implementation details. | Pending | In Progress |
| Add focused test helpers for key feeding, command-line feeding, and pane state assertions so tests are readable without hiding the user action being tested. | Planned: add concise helpers in regression tests or support modules. | Planned: converted fed-key tests should use the helpers while keeping visible key strings/commands in the test body. | This roadmap; no public docs change expected because helpers are internal. | Confirm test failures name the user action clearly. | Pending | In Progress |
| Run focused ask tests, `tests/run_checks.sh fast`, `tests/run_checks.sh full`, `illu.nvim` smoke when mappings/local integration are affected, and `git diff --check`. | Planned: execute these checks before slice closeout. `illu.nvim` smoke will run only if this test-only slice changes defaults, mappings, commands, public API, local config behavior, or local integration. | Planned: record exact check outcomes under this slice. | This roadmap records verification; no public docs change expected unless behavior changes. | Run the same checks locally if desired. | Pending | In Progress |
...
| For every mapping listed in the behavior-sensitive coverage table, perform the real keypress in Neovim and compare the outcome to the matrix. | Planned: manual acceptance row maps to the behavior-sensitive coverage table. | Not Applicable as automated test: this bullet is itself a manual acceptance requirement, supported by fed-key regressions. | Coverage table produced by this slice plus existing behavior/mapping matrices. | Perform this exact manual workflow. | Pending | In Progress |
| Repeat personal `qq` / `<leader>qq` checks in Markdown, Codex, and ask panes. | Planned: guarded quit mappings and ask send mappings remain covered by fed-key tests/table entries. | Planned: fed-key tests should cover personal quit mappings in non-ask panes and ask send paths in ask panes. | Existing mapping docs remain unchanged unless behavior changes. | Perform this exact manual workflow. | Pending | In Progress |
| Repeat `<C-CR>` / `<C-J>` submit checks from normal and insert ask-pane modes. | Planned: submit fed-key coverage should cover normal/insert submit paths and fallback mapping where synthesizeable. | Planned: fed-key submit tests should cover these manual workflows or document any synthesis limitation. | Existing mapping docs remain unchanged unless behavior changes. | Perform this exact manual workflow. | Pending | In Progress |
| Run the focused ask test group and confirm failures point to user behavior, not just callback plumbing. | Planned: focused ask tests should be named/grouped by behavior layer and user path. | Planned: focused ask test output will be recorded in verification results. | This roadmap; no public docs change expected because this is test architecture. | Perform this exact manual workflow. | Pending | In Progress |
