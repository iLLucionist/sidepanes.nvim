...
| `SidepanesAsk`, `SidepanesAskAppend`, `SidepanesSubmitQuestion`, `SidepanesAskStatus`, and `SidepanesVersion` once implemented. | Matrix rows `ask-zone-commands` and `ask-zone-planned-commands`; fixture required commands now include `SidepanesAskStatus`, while planned commands contain only `SidepanesVersion`. | Zone matrix regression verifies current commands are registered and `SidepanesVersion` is not registered early; docs contract verifies the remaining planned command row exists. | This roadmap matrix. | Manual command checks for implemented commands; only `SidepanesVersion` waits for slice 21. | Done |
