...
| --- | --- | --- | --- | --- | --- | --- |
| Define one ask session model or snapshot module that owns the public shape of ask state consumed by policy, lifecycle, winbar, status, health/debug output, and future commands. | `lua/sidepanes/ask_session.lua` defines the snapshot boundary, lifecycle fact projection, status data, title formatter, and state-history helper. Runtime consumers still need to move to it. | `tests/sidepanes_regression.lua` "ask session snapshot exposes serializable state facts and labels", "ask session snapshot covers empty invalid target and picker cases", and purity check passed in focused regression. Integration tests still planned. | This roadmap; no public docs change applies yet because no user-visible behavior changed. | Manual snapshot/state agreement checks listed under this slice. | `0c1054c` | In Progress |
| Keep raw mutable Neovim/session state separate from pure snapshots: | `ask_session.snapshot()` accepts raw ask state plus explicit buffer/window/config facts and returns a serializable table. Runtime state still needs to route through it. | Direct snapshot tests cover fake buffer IDs, target data, cached written prompts, explicit buffer/window facts, and the functional-core no-`vim.*` purity check. | This roadmap architecture requirement; no public docs change applies because this unit is internal. | Manual architecture review plus visible lifecycle checks. | `0c1054c` | In Progress |
...
| policy consumes snapshots or facts, not raw Neovim objects. | `ask_session.lifecycle_facts()` now exists; `ask_pane.lua` still needs to use it for controller facts. | Direct lifecycle-fact projection is tested; runtime policy integration tests still planned. | This roadmap; no public docs change applies because policy inputs are internal. | Manual lifecycle behavior checks. | `0c1054c` | Planned |
...
| active/inactive ask session. | `ask_session.snapshot()` reports `active` from raw session presence plus explicit valid-buffer facts. | Direct selector tests cover ready active sessions and invalid-buffer inactive sessions; reset/sent/cancelled runtime integration still planned. | This roadmap; future `SidepanesAskStatus` docs will use this output. | Open/cancel/send ask pane and compare active/inactive status. | `0c1054c` | In Progress |
...
| valid buffer/window facts. | `ask_session.snapshot()` reports `valid_buffer`, `valid_window`, and `active_window` from explicit facts supplied by the shell. | Direct selector tests cover valid buffer/window, invalid window, and invalid-buffer inactive cases. Missing/deleted runtime cases still planned. | This roadmap; no public docs change applies because validity facts are internal. | Manual invalid-window behavior review if a reproducible interaction exists. | `0c1054c` | In Progress |
| modified/written/live prompt facts. | `ask_session.snapshot()` reports `dirty_buffer`, `live_prompt`, and `written_prompt`; `lifecycle_facts()` projects them for policy. | Direct selector tests cover live vs written prompts, empty ready prompt, modified prompt, and invalid-buffer preserved facts. Runtime edited-after-write and failed-send integration still planned. | Existing public lifecycle docs should remain accurate unless behavior changes. | Append/edit/write/submit/cancel workflow listed under this slice. | `0c1054c` | In Progress |
...
| Move state history and last-state reporting toward the ask session/snapshot boundary instead of storing lifecycle history partly beside `state.ask_pane`. | `ask_session.record_state()` owns lifecycle history/last-state mutation; `ask_pane.lua` still needs to delegate to it. | Direct state-history test covers `record_state()`; runtime lifecycle transition tests still need to prove ask-pane callers use it. | Existing lifecycle docs remain unchanged; roadmap records the internal move. | Inspect state labels through visible winbar/status-facing output during lifecycle workflows. | `0c1054c` | In Progress |
